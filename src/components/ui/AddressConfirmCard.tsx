...
export default function AddressConfirmCard({ address, name, phone, onChangeClick }: AddressConfirmCardProps) {
  // Use values from address first, fallback to props
  const displayName = address.full_name || name || 'Customer';
  const displayPhone = address.phone || phone;
  const formattedPhone = displayPhone ? (displayPhone.startsWith('+91') ? displayPhone : `+91 ${displayPhone.replace(/^(\d{5})/, '$1 ')}`) : '';
    ...
    <div className="rounded-xl border border-[rgba(201,168,76,0.2)] bg-[#0D1B2A] p-5 relative overflow-hidden group transition-colors hover:border-[#C9A84C]">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-medium text-[#F5F0E8] tracking-widest uppercase">📍 Delivering to</h3>
        </div>
      ...
      <div className="pl-7 space-y-1">
        <p className="text-[15px] text-[#F5F0E8] font-medium">{displayName}</p>
        <p className="text-sm text-[#F5F0E8]/90 leading-relaxed">
          {address.street}
          {address.landmark && `, ${address.landmark}`}
          <br />
          {address.city}, {address.state} &middot; {address.pincode}
          <br />
          {address.country}
        </p>
        {displayPhone && (
